...
resource "aws_autoscaling_policy" "sensor_autoscale_scale_in_policy" {
...
resource "awscc_cloudwatch_alarm" "sensor_asg_low_cpu_alarm" {
  ...
  namespace           = "AWS/EC2"
  alarm_actions       = [aws_autoscaling_policy.sensor_autoscale_scale_in_policy.arn]
  dimensions = [
    {
      name  = "AutoScalingGroupName"
