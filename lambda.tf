...
resource "aws_lambda_function" "multiple_eni_auto_scaling_lambda" {
  function_name = var.lambda_function_name
  role          = aws_iam_role.multi_eni_role.arn
...
resource "aws_cloudwatch_log_group" "log_group" {
  name              = "${var.cloudwatch_log_group_prefix}/${aws_lambda_function.multiple_eni_auto_scaling_lambda.function_name}"
...
resource "aws_cloudwatch_event_target" "ec2_state_change_rule_lambda_target" {
  arn  = aws_lambda_function.multiple_eni_auto_scaling_lambda.arn
...
resource "aws_lambda_permission" "ec2_state_change_event_bridge_trigger_permission" {
  ...
  function_name = aws_lambda_function.multiple_eni_auto_scaling_lambda.function_name
